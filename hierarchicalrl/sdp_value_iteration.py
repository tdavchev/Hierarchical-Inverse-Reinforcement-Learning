    ...
    while diff > threshold:
        ...
        for state in option_states:
            ...
            for action in range(n_actions):
                ...
                max_v = max(max_v, sum(reward + np.dot(transition_p, (discount * value))))
                # max_v = max(max_v, np.dot(tp, reward + discount*v))

            new_diff = abs(value[state] - max_v)
            if new_diff > diff:
                diff = new_diff
            value[state] = max_v
...
def find_policy(options_states, n_states, n_actions, transition_probabilities, reward, discount,
    ...
    if v is None:
        v = []
        for option_states in options_states:
            v.append(optimal_value(option_states, n_actions, transition_probabilities, reward,
                     discount, threshold))
